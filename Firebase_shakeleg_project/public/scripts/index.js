...
function epochToJsDate(epochTime){
  ...
  const sittingperhourElement = document.getElementById("sittingperhour");
  const shakingperhourElement = document.getElementById("shakingperhour");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          ...
          var sit = jsonData.sit;
          var shake = jsonData.shake;

          var timestamp = jsonData.timestamp;

          // Plot the values on the charts
          plotValues(chartW, timestamp, sit);
          plotValues(chartS, timestamp, shake);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var sittingperhour = jsonData.sit;
        var shakingperhour = jsonData.shake;
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        sittingperhourElement.innerHTML = sittingperhour;
        shakingperhourElement.innerHTML = shakingperhour;
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        var gaugeT = createSittingGauge();
        gaugeT.draw();
        gaugeT.value = sittingperhour;
        updateElement.innerHTML = epochToDateTime(timestamp);
      });
